-- =========================================================================
-- SIKon — Fix distribute_sewing_work: filter penjahit berdasarkan role
-- =========================================================================
-- Masalah: fungsi sebelumnya mengambil SEMUA staff wage_type='piecework',
--          termasuk tukang potong yang juga bertipe piecework.
-- Fix: tambahkan filter role agar hanya staf dengan role "Penjahit" /
--      "Sewing" / "jahit" yang mendapat pembagian kerja jahit.
--      Jika tidak ada staf dengan role jahit yang aktif, fallback ke
--      semua staff piecework aktif (agar tidak error di setup baru).
-- =========================================================================
...
begin
  ...
  -- ── Buat batch ───────────────────────────────────────────────────────────
  insert into public.sewing_distribution_batches (user_id, notes, total_qty, staff_count)
  values (v_user_id, p_notes, v_pool_total, v_staff_count)
  ...
  for v_pool in
    ...
    order by oi.ready_for_sewing_at asc
  loop
    ...
    while v_pool_item_remaining > 0 loop
      ...
      insert into public.sewing_assignments
        (user_id, order_item_id, staff_id, batch_id, assigned_qty, status, notes)
      values
        (v_user_id, v_pool.order_item_id, v_staff_ids[v_s], v_batch_id,
         v_assign_qty, 'assigned', p_notes);
